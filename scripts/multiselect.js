...
if (typeof ($) != 'undefined') {
	...
	$.fn.multiselect = function (options) {
		...
		if (options.placeholder === undefined) options.placeholder = "Please Select";
		var res = [];
		...
		if (this.length != 0) {
			$(this).each(function (i, e) {
				...
				if (index == -1) {
					var inputItem = new Multiselect(e, options.placeholder);
	...
	document.multiselect = function(selector, options) {
		if (options === undefined) options = {};
		if (options.placeholder === undefined) options.placeholder = "Please Select";
		var res = [];
		if (!window.multiselects._items) {
